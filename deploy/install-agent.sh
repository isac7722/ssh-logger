...
if [[ $agent_arch == arm64 ]]; then rules="$base/deploy/audit-arm64.rules"; else rules="$base/deploy/audit.rules"; fi
if [[ -e /etc/audit/rules.d/ssh-logger.rules ]] && ! cmp -s "$rules" /etc/audit/rules.d/ssh-logger.rules; then
...
install -m 640 "$rules" /etc/audit/rules.d/ssh-logger.rules
augenrules --load
