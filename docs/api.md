...
Super admin만 계정 목록·생성·삭제·권한 배정 API에 접근한다. 일반 관리자는 자신의 비밀번호를 변경할 수 있다. 아래 API는 로그인 세션이 필요하며 변경 요청은 Origin·CSRF 검사를 적용한다.
...
| `DELETE /api/admins/{username}` | Super admin 전용 일반 관리자 삭제. 성공 200 및 `{"ok":true}`. 일반 관리자 요청·최고 관리자 대상은 403, 없는 계정은 404 |
...
삭제는 계정·역할·서버 배정·해당 계정의 모든 인증 세션을 하나의 트랜잭션에서 제거한다. 요청자 권한과 세션 유효성, 삭제 대상의 현재 역할을 트랜잭션 안에서 재검사하며, 최고 관리자는 본인과 다른 최고 관리자 모두 삭제할 수 없다. 삭제된 계정은 이후 인증 요청과 로그인이 거부된다. 기존 SSH 연결·수집 기록·방화벽 및 세션 종료 요청 이력은 보존한다. 저장 실패 시 503이며 세션·배정 삭제도 롤백된다. 별도 스키마 변경은 없다.
